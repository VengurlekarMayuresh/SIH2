...
async function createEarthquakeQuiz() {
    try {
        ...
        const modules = await Module.find({}, 'title _id');
        console.log('\n📚 Available Modules:');
        modules.forEach(module => {
            console.log(`- ${module.title} (ID: ${module._id})`);
        });
        
        // Let's use the first available module, or create a default one if none exist
        let moduleId;
        if (modules.length > 0) {
            moduleId = modules[0]._id;
            console.log(`\n✅ Using module: ${modules[0].title}`);
        } else {
            // Create a default disaster safety module
            const newModule = new Module({
                title: "Earthquake Safety",
                description: "Learn about earthquake preparedness and safety measures",
                thumbnail: "https://example.com/earthquake-thumbnail.jpg",
                chapters: []
            });
            await newModule.save();
            moduleId = newModule._id;
            console.log('\n✅ Created new module: Earthquake Safety');
        }
